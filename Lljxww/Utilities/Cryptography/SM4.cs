using Org.BouncyCastle.Utilities.Encoders;
using System;
using System.Collections.Generic;
using System.Text;

namespace Lljxww.Utilities.Cryptography;

public class SM4
{
    private static readonly Encoding encoding = Encoding.UTF8;

    /// <summary>
    /// 加密ECB模式
    /// </summary>
    /// <param name="secretKey">密钥</param>
    /// <param name="hexString">明文是否是十六进制</param>
    /// <param name="plainText">明文</param>
    /// <param name="isBase64">返回密文编码格式</param>
    /// <returns>返回密文</returns>
    public static string EncryptECB(string secretKey, bool hexString, string plainText, bool isBase64)
    {
        if (string.IsNullOrWhiteSpace(plainText))
        {
            return string.Empty;
        }

        Sm4Context ctx = new()
        {
            IsPadding = true,
            Mode = Sm4.SM4_ENCRYPT
        };

        byte[] keyBytes = hexString ? Hex.Decode(secretKey) : encoding.GetBytes(secretKey);

        Sm4 sm4 = new();
        sm4.SetkeyENC(ctx, keyBytes);
        byte[] encrypted = sm4.CryptECB(ctx, encoding.GetBytes(plainText));
        byte[]? encode = isBase64 ? Base64.Encode(encrypted) : Hex.Encode(encrypted);
        string cipherText = encoding.GetString(encode);
        return cipherText;
    }

    /// <summary>
    /// 解密ECB模式
    /// </summary>
    /// <param name="secretKey">密钥</param>
    /// <param name="hexString">明文是否是十六进制</param>
    /// <param name="cipherText">密文</param>
    /// <param name="isBase64">输入密文编码格式</param>
    /// <returns>返回明文</returns>
    public static string DecryptECB(string secretKey, bool hexString, string cipherText, bool isBase64)
    {
        try
        {
            Sm4Context ctx = new()
            {
                IsPadding = true,
                Mode = Sm4.SM4_DECRYPT
            };

            byte[] keyBytes = hexString ? Hex.Decode(secretKey) : encoding.GetBytes(secretKey);

            Sm4 sm4 = new();
            sm4.SetkeyDEC(ctx, keyBytes);
            byte[]? encode = isBase64 ? Base64.Decode(cipherText) : Hex.Decode(cipherText);

            byte[] decrypted = sm4.CryptECB(ctx, encode);
            return encoding.GetString(decrypted);
        }
        catch (Exception)
        {
            return "";
        }
    }

    /// <summary>
    /// 加密CBC模式
    /// </summary>
    /// <param name="secretKey">密钥</param>
    /// <param name="hexString">明文是否是十六进制</param>
    /// <param name="iv"></param>
    /// <param name="plainText">明文</param>
    /// <returns>返回密文</returns>
    public static string EncryptCBC(string secretKey, bool hexString, string iv, string plainText)
    {
        Sm4Context ctx = new()
        {
            IsPadding = true,
            Mode = Sm4.SM4_ENCRYPT
        };

        byte[] keyBytes;
        byte[] ivBytes;
        if (hexString)
        {
            keyBytes = Hex.Decode(secretKey);
            ivBytes = Hex.Decode(iv);
        }
        else
        {
            keyBytes = encoding.GetBytes(secretKey);
            ivBytes = encoding.GetBytes(iv);
        }

        Sm4 sm4 = new();
        sm4.SetkeyENC(ctx, keyBytes);
        byte[] encrypted = sm4.CryptCBC(ctx, ivBytes, encoding.GetBytes(plainText));
        string cipherText = encoding.GetString(Hex.Encode(encrypted));
        return cipherText;
    }

    /// <summary>
    /// 解密CBC模式
    /// </summary>
    /// <param name="secretKey">密钥</param>
    /// <param name="hexString">明文是否是十六进制</param>
    /// <param name="iv"></param>
    /// <param name="cipherText">密文</param>
    /// <returns>返回明文</returns>
    public static string DecryptCBC(string secretKey, bool hexString, string iv, string cipherText)
    {
        try
        {
            Sm4Context ctx = new()
            {
                IsPadding = true,
                Mode = Sm4.SM4_DECRYPT
            };

            byte[] keyBytes;
            byte[] ivBytes;
            if (hexString)
            {
                keyBytes = Hex.Decode(secretKey);
                ivBytes = Hex.Decode(iv);
            }
            else
            {
                keyBytes = encoding.GetBytes(secretKey);
                ivBytes = encoding.GetBytes(iv);
            }

            Sm4 sm4 = new();
            sm4.SetkeyDEC(ctx, keyBytes);
            byte[] decrypted = sm4.CryptCBC(ctx, ivBytes, Hex.Decode(cipherText));
            return encoding.GetString(decrypted);
        }
        catch (Exception)
        {
            return "";
        }
    }
}

internal class Sm4
{
    public const int SM4_ENCRYPT = 1;
    public const int SM4_DECRYPT = 0;

    private readonly uint[] CK =
    [
        0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
        0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
        0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
        0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
        0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
        0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
        0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
        0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
    ];

    private readonly uint[] FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc];

    private readonly byte[] S_BOX_TABLE =
    [
        
    ];

    private static long GetUlongBE(byte[] b, int i)
    {
#pragma warning disable CS0675 // 对进行了带符号扩展的操作数使用了按位或运算符
        return ((long)(b[i] & 0xff) << 24)
            | ((b[i + 1] & 0xff) << 16)
            | ((b[i + 2] & 0xff) << 8)
            | (b[i + 3] & 0xff & 0xffffffffL);
#pragma warning restore CS0675 // 对进行了带符号扩展的操作数使用了按位或运算符
    }

    private static void PutUlongBE(long n, byte[] b, int i)
    {
        b[i] = (byte)(int)(0xFF & (n >> 24));
        b[i + 1] = (byte)(int)(0xFF & (n >> 16));
        b[i + 2] = (byte)(int)(0xFF & (n >> 8));
        b[i + 3] = (byte)(int)(0xFF & n);
    }

    private static long SHL(long x, int n)
    {
        return (x & 0xFFFFFFFF) << n;
    }

    private static long ROTL(long x, int n)
    {
        return SHL(x, n) | (x >> (32 - n));
    }

    private static void SWAP(long[] sk, int i)
    {
        (sk[31 - i], sk[i]) = (sk[i], sk[31 - i]);
    }

    private byte SBox(byte inch)
    {
        int i = inch & 0xFF;
        byte retVal = S_BOX_TABLE[i];
        return retVal;
    }

    private long Lt(long ka)
    {
        byte[] a = new byte[4];
        byte[] b = new byte[4];
        PutUlongBE(ka, a, 0);
        b[0] = SBox(a[0]);
        b[1] = SBox(a[1]);
        b[2] = SBox(a[2]);
        b[3] = SBox(a[3]);
        long bb = GetUlongBE(b, 0);
        long c = bb ^ ROTL(bb, 2) ^ ROTL(bb, 10) ^ ROTL(bb, 18) ^ ROTL(bb, 24);
        return c;
    }

    private long F(long x0, long x1, long x2, long x3, long rk)
    {
        return x0 ^ Lt(x1 ^ x2 ^ x3 ^ rk);
    }

    private long CalciRK(long ka)
    {
        byte[] a = new byte[4];
        byte[] b = new byte[4];
        PutUlongBE(ka, a, 0);
        b[0] = SBox(a[0]);
        b[1] = SBox(a[1]);
        b[2] = SBox(a[2]);
        b[3] = SBox(a[3]);
        long bb = GetUlongBE(b, 0);
        long rk = bb ^ ROTL(bb, 13) ^ ROTL(bb, 23);
        return rk;
    }

    private void SetKey(long[] SK, byte[] key)
    {
        long[] MK = new long[4];
        long[] k = new long[36];
        int i = 0;
        MK[0] = GetUlongBE(key, 0);
        MK[1] = GetUlongBE(key, 4);
        MK[2] = GetUlongBE(key, 8);
        MK[3] = GetUlongBE(key, 12);
        k[0] = MK[0] ^ FK[0];
        k[1] = MK[1] ^ FK[1];
        k[2] = MK[2] ^ FK[2];
        k[3] = MK[3] ^ FK[3];
        for (; i < 32; i++)
        {
            k[i + 4] = k[i] ^ CalciRK(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i]);
            SK[i] = k[i + 4];
        }
    }

    private void OneRound(long[] sk, byte[] input, byte[] output)
    {
        int i = 0;
        long[] ulBuf = new long[36];
        ulBuf[0] = GetUlongBE(input, 0);
        ulBuf[1] = GetUlongBE(input, 4);
        ulBuf[2] = GetUlongBE(input, 8);
        ulBuf[3] = GetUlongBE(input, 12);
        while (i < 32)
        {
            ulBuf[i + 4] = F(ulBuf[i], ulBuf[i + 1], ulBuf[i + 2], ulBuf[i + 3], sk[i]);
            i++;
        }

        PutUlongBE(ulBuf[35], output, 0);
        PutUlongBE(ulBuf[34], output, 4);
        PutUlongBE(ulBuf[33], output, 8);
        PutUlongBE(ulBuf[32], output, 12);
    }

    private static byte[] Padding(byte[] input, int mode)
    {
        byte[] ret;
        if (mode == SM4_ENCRYPT)
        {
            int p = 16 - (input.Length % 16);
            ret = new byte[input.Length + p];
            Array.Copy(input, 0, ret, 0, input.Length);
            for (int i = 0; i < p; i++)
            {
                ret[input.Length + i] = (byte)p;
            }
        }
        else
        {
            int p = input[^1];
            ret = new byte[input.Length - p];
            Array.Copy(input, 0, ret, 0, input.Length - p);
        }

        return ret;
    }

    public void SetkeyENC(Sm4Context ctx, byte[] key)
    {
        ctx.Mode = SM4_ENCRYPT;
        SetKey(ctx.SK, key);
    }

    public void SetkeyDEC(Sm4Context ctx, byte[] key)
    {
        ctx.Mode = SM4_DECRYPT;
        SetKey(ctx.SK, key);
        int i;
        for (i = 0; i < 16; i++)
        {
            SWAP(ctx.SK, i);
        }
    }

    public byte[] CryptECB(Sm4Context ctx, byte[] input)
    {
        if (ctx is { IsPadding: true, Mode: SM4_ENCRYPT })
        {
            input = Padding(input, SM4_ENCRYPT);
        }

        int length = input.Length;
        byte[] bins = new byte[length];
        Array.Copy(input, 0, bins, 0, length);
        byte[] bous = new byte[length];
        for (int i = 0; length > 0; length -= 16, i++)
        {
            byte[] inBytes = new byte[16];
            byte[] outBytes = new byte[16];
            Array.Copy(bins, i * 16, inBytes, 0, length > 16 ? 16 : length);
            OneRound(ctx.SK, inBytes, outBytes);
            Array.Copy(outBytes, 0, bous, i * 16, length > 16 ? 16 : length);
        }

        if (ctx is { IsPadding: true, Mode: SM4_DECRYPT })
        {
            bous = Padding(bous, SM4_DECRYPT);
        }

        return bous;
    }

    public byte[] CryptCBC(Sm4Context ctx, byte[] iv, byte[] input)
    {
        if (ctx is { IsPadding: true, Mode: SM4_ENCRYPT })
        {
            input = Padding(input, SM4_ENCRYPT);
        }

        int i = 0;
        int length = input.Length;
        byte[] bins = new byte[length];
        Array.Copy(input, 0, bins, 0, length);
        List<byte> bousList = [];
        if (ctx.Mode == SM4_ENCRYPT)
        {
            for (int j = 0; length > 0; length -= 16, j++)
            {
                byte[] inBytes = new byte[16];
                byte[] outBytes = new byte[16];
                byte[] out1 = new byte[16];
                Array.Copy(bins, i * 16, inBytes, 0, length > 16 ? 16 : length);
                for (i = 0; i < 16; i++)
                {
                    outBytes[i] = (byte)(inBytes[i] ^ iv[i]);
                }

                OneRound(ctx.SK, outBytes, out1);
                Array.Copy(out1, 0, iv, 0, 16);
                for (int k = 0; k < 16; k++)
                {
                    bousList.Add(out1[k]);
                }
            }
        }
        else
        {
            byte[] temp = new byte[16];
            for (int j = 0; length > 0; length -= 16, j++)
            {
                byte[] inBytes = new byte[16];
                byte[] outBytes = new byte[16];
                byte[] out1 = new byte[16];

                Array.Copy(bins, i * 16, inBytes, 0, length > 16 ? 16 : length);
                Array.Copy(inBytes, 0, temp, 0, 16);
                OneRound(ctx.SK, inBytes, outBytes);
                for (i = 0; i < 16; i++)
                {
                    out1[i] = (byte)(outBytes[i] ^ iv[i]);
                }

                Array.Copy(temp, 0, iv, 0, 16);
                for (int k = 0; k < 16; k++)
                {
                    bousList.Add(out1[k]);
                }
            }
        }

        if (!ctx.IsPadding || ctx.Mode != SM4_DECRYPT)
        {
            return [.. bousList];
        }

        byte[] bous = Padding([.. bousList], SM4_DECRYPT);
        return bous;
    }
}

internal class Sm4Context
{
    public bool IsPadding;
    public int Mode;

    public long[] SK;

    public Sm4Context()
    {
        Mode = 1;
        IsPadding = true;
        SK = new long[32];
    }
}