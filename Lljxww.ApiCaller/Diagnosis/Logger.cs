...
public static class Logger
{
    public static void Log(CallerContext context)
    {
        ...
        _ = sb.Append($"Parameters: {JsonSerializer.Serialize(context.RequestContext.Param)}{Environment.NewLine}");
        _ = sb.Append($"Result: {context.ApiResult?.RawStr}");
        _ = sb.Append($"RunTime: {context.Runtime}{Environment.NewLine}");
        _ = sb.Append($"Authorization Info: {context.RequestContext.CustomAuthorizeInfo}{Environment.NewLine}");
